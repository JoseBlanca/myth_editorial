...
# Input file: chapters/31-vainamoinen-iku-turso.claims.approved.adoc

```
[[ch-31-vainamoinen-iku-turso-claims]]
== Väinämöinen and Iku-Turso (Transformed Variant) — Claims

// One fact per paragraph; chapter-draft will transform this into narrative.
// Related-tier (full, transformed-variant) chapter on the Finnic encounter
// in which the sea-monster Iku-Turso emerges from the sea and is subdued —
// not slain — by the shaman-sage hero Väinämöinen by means of song-and-
// knowledge rather than by weapon. Per brief 31-vainamoinen-iku-turso.yaml
// the chapter is "related (full, transformed-variant framing)" with a
// target of approximately 4,500 narrative words. Variant handling is co-
// equal: the older rune-tradition witnesses (laconic, fragmentary,
// recorded by 19th-century rune-collectors from Karelian and Finnish
// singers) are presented alongside Lönnrot's 1835 / 1849 Kalevala
// editorial synthesis (composite, expanded, integrated into the modern
// national epic). The two layers are kept explicitly distinct rather
// than collapsed, per scope-lock 2026-04-23 "transformed variant"
// framing and per brief special_instructions. Siikala 2002 (Mythic
// Images and Shamanism, FFC 280) is the chapter's principal area-
// specialist anchor; Honko and DuBois are named-but-flagged in body
// prose as the broader Lönnrot-method scholarship per the Ch23 FC-02 /
// Ch26 FC-02 / Ch27 / Ch28 / Ch30 named-but-flagged precedent. The
// chapter opens with the Finnic tradition's own framing — Väinämöinen's
// shaman-sage character, the song-and-knowledge register of the
// encounter, the binding-rather-than-slaying structure — before
// invoking the comparative chaoskampf category, and explicitly
// registers (per brief special_instructions) that the chapter does
// NOT claim Iku-Turso fits chaoskampf cleanly. The chapter contributes
// to Part II C7 (the inversion / transformation problem; ch. 40) as a
// PRINCIPAL CASE for the *transformation* sub-problem and to Part II
// C8 (the refusal and the appropriation problem; ch. 41) for the
// methodological point about 19th-century editorial synthesis.
...
// Checks passed: footnote+evidence-pair (38/38), source_id-whitelist, glossary-discipline, markers-valid
// Fixes applied: FC-01 HIGH (CITATION-WRONG, canto location for Iku-Turso's principal Kalevala appearance at body-prose line 133 — MODIFY per task-instructions guidance: the auditor flagged the chapter's "50th canto" assertion as wrong; per Wikipedia *Iku-Turso (creature)* and *Cantos of the Kalevala*, Iku-Turso appears principally in Cantos II (2) and XLII (42) of the New Kalevala, with Canto 42 being the Sampo-cycle subjugation episode the chapter correctly describes elsewhere and Canto 50 being the Marjatta and Väinämöinen's-departure final canto unrelated to the Iku-Turso encounter. Body prose at line 133 revised from "the *Kalevala*'s 50th canto (in the 1849 *Uusi Kalevala* numbering) and in adjacent passages connected with the Sampo-cycle and the Pohjola material" to "the *Kalevala*'s 42nd canto (Rune XLII, in the 1849 *Uusi Kalevala* numbering — the Sampo-cycle and Pohjola material in which Louhi summons Iku-Turso to stop the Kalevala heroes' return with the Sampo and Väinämöinen subdues him by song), with an earlier brief appearance in the 2nd canto (the rising-from-the-sea-and-burning-the-hay episode that produces the great oak)" per the auditor's recommended_fix; matching footnote tightened identically with explicit Louhi-summons-and-Väinämöinen-binds-by-the-ears narrative detail per the auditor's evidence note. No EVIDENCE-token disposition change since the source_id siikala-finnish was already correctly the anchor; the canto-number claim is brought into alignment with what the Kalevala source actually contains.); FC-02 HIGH (CITATION-WRONG, Kalevalan päivä commemoration date and edition at body-prose line 139 — MODIFY per task-instructions guidance: the auditor flagged the chapter's "1849 publication date — 28 February 1849" formulation as conflating two distinct facts; per Wikipedia *Kalevala Day*, the date 28 February commemorates Lönnrot's signing the preface to the FIRST edition (1835 Vanha Kalevala), NOT the 1849 New Kalevala — whose preface is dated April 1849. Body prose at line 139 revised from "since the 1849 *Uusi Kalevala* (with the publication date — 28 February 1849 — celebrated annually in Finland as *Kalevalan päivä*, 'Kalevala Day', and as the Day of Finnish Culture)" to "since the 1835 *Vanha Kalevala* and especially since the 1849 *Uusi Kalevala* became the standard text (with the date 28 February 1835, on which Lönnrot signed the preface to the first edition, celebrated annually in Finland as *Kalevalan päivä*, 'Kalevala Day', and as the Day of Finnish Culture)" per the auditor's recommended_fix; matching footnote tightened identically with explicit "Kalevala Day commemorates Lönnrot's signing of the 1835 preface, not the 1849 edition's publication date, which is in April 1849" qualifier per the auditor's evidence note. No EVIDENCE-token disposition change since the source_id siikala-finnish was already correctly the anchor; the commemoration-date-and-edition claim is brought into alignment with what the Kalevala Day commemoration actually celebrates.); FC-03 LOW (identifier_year_ambiguity, Siikala 2002 vs Folklore Fellows official catalogue 2003 in sources.yaml siikala-finnish entry) — NOTE-only / DEFER per task-instructions guidance ("your call — either ACCEPT the year correction in sources.yaml, or NOTE-only and defer"): the 2002 dating in sources.yaml is defensible per multiple library catalogues (Open Library, Project MUSE, Semantic Scholar) and matches the predominant secondary-source convention; the FFC official page lists 2003, apparently a late-2002 publication with general circulation beginning in 2003. The discrepancy is a minor bibliographic ambiguity that does not affect any claim's accuracy; pinned ISBN 9789514109027 is verified correct per the auditor's plausibility check. The year-ambiguity is left at NOTE-only with no sources.yaml change at this stage; if the user prefers the FFC official-catalogue 2003 dating, the sources.yaml entry's year field can be updated at inventory-audit. INFO-01 (LACUNA marker reference to "+ index volumes" for SKVR at line 59) and INFO-02 (LACUNA marker reference to "SKS Folklore Archives" historical name) — NOTE-only per task-instructions guidance: both are minor naming-convention imprecisions that the auditor explicitly classified as INFO-level non-finding observations. The LACUNA marker text is left unchanged; the substance of the references is correct.
// Sources.yaml additions: 0 new entries for Ch31. The siikala-finnish entry's year field is left at 2002 per the FC-03 NOTE-only disposition. Source count unchanged from the Ch29/Ch30 cycle (69 entries → 69 entries before the Ch32 in-place updates documented in the Ch32 approved file).
// Briefs / governing-document changes: 0 brief edits for Ch31 (no FC-finding required brief-level acknowledgement at this stage — FC-01 is a body-prose-and-footnote canto-number correction not encoded at the brief level; FC-02 is a body-prose-and-footnote commemoration-date-and-edition correction not encoded at the brief level; FC-03 is the deferred year-ambiguity NOTE on sources.yaml with no brief-level counterpart; INFO-01 and INFO-02 are LACUNA-marker minor naming notes deferred). Brief 31-vainamoinen-iku-turso.yaml unchanged.
// Warnings: none. Banned umbrella terms ("chaos-figure", "chaos-monster", "chaos-serpent") not present in body prose, footnotes, markers, or COMPARATIVE-HOOK comments. Glossary discipline holds (Väinämöinen, Iku-Turso, Lönnrot, Kalevala, Vanha Kalevala, Uusi Kalevala, Kalevalan päivä, Karelia, Viena Karelia, Aunus, Olonets, Ingria, Pohjola, Louhi, Ilmarinen, Lemminkäinen, Sampo, runonlaulajat, Suomalainen Kirjallisuuden Seura, SKS, Suomen Kansan Vanhat Runot, SKVR, Siikala, Honko, DuBois, Akseli Gallen-Kallela, Jean Sibelius, Mikael Agricola, Arhippa Perttunen, Latvajärvi, Ontrei Malinen, Vuonninen, Daniel Europaeus, August Ahlqvist, Arvid Genetz, Krohn, Forsyth, Macpherson, Ossian, Ivanov, Toporov, Perun, Veles in conventional scholarly forms throughout). Forsyth disclaimer wording at line 53 verified as correct per the framing-only (non-exception) precedent — the single Forsyth EVIDENCE token correctly characterises Forsyth as the long-arc combat-myth typology framing anchor with explicit "Forsyth 1987 does not itself treat Finnic material directly, and the chapter's use of the citation is for the long-arc typological framing rather than for Finnic-area content scholarly coverage" disclaimer; no further fix required. APPROPRIATION-FLAG marker at line 158 verified as complete per the named-area-specialist-plus-objection-substance-plus-tier-implication-plus-spectrum-positioning-plus-marker-placement five-component checklist (Siikala named in first factual pass; Honko and DuBois named-but-flagged with explicit deferral-to-inventory-audit per Ch23 FC-02 / Ch26 FC-02 / Ch27 / Ch28 / Ch30 precedents; substance of typological-assimilation objection via shamanic-and-knowledge register and Lönnrot-method critical framework; related-tier transformed-variant implication consistent with PRINCIPAL CASE for Part II C7 *transformation* sub-problem; spectrum positioning at mid-spectrum alongside Ch23 Susanoo and Ch26 Pariacaca and milder than Ch29 Mwindo and Ch32 Nanabozho refusals; structural marker placement at consolidated typology-and-forward-links section per Ch27 / Ch28 / Ch30 marker-placement template). Two-layer presentation discipline (older rune-tradition material first, Lönnrot's 1835 / 1849 *Kalevala* editorial synthesis second) verified as preserved throughout; transformed-variant framing preserved at the consolidated typology section. The original 31-vainamoinen-iku-turso.claims.adoc file is confirmed UNMODIFIED per the Ch27/Ch28-cycle-closed and Ch29/Ch30-precedent constraint (do NOT modify originals; fixes go ONLY into the .claims.approved.adoc copy).
```

---

# Input file: chapters/31-vainamoinen-iku-turso.adoc
